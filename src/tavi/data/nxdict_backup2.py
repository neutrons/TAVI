...
class NexusDict(object):
    ...
    format into NeXus style:
    nxentry_dict = {
        "attrs": {"attr1": attr1, ...},
        "key1": {
            "dataset": dataset,
            "attrs": {"ds_attr1": ds_attr1, ...},
        },
    }
...
def spicelogs_to_nested_dict(
    ...
    name = NexusDict()
    name.set_attrs(type="NX_CHAR", EX_required="true")
    probe = NexusDict()
    probe.set_attrs(type="NX_CHAR", EX_required="true")
    nxsource = NexusDict()
    nxsource.set_attrs(NX_class="NXsource", EX_required="true", name=name, probe=probe)

    # name.set_dataset(key="name", dataset="HFIR")
    # nxsource.set_dataset(key="probe", dataset="neutron")
