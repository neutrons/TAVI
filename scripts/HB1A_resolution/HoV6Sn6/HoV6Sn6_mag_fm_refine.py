...
def export_intensity(file_name):
    (hkl_list, rez_list, _, (_, exp_s1_q_list), _) = analysis
    lorentz_factor_list = [
        rez.r0 * np.sqrt((rez.mat[0, 0] * rez.mat[1, 1] - rez.mat[0, 1] * rez.mat[1, 0]) / rez.mat[1, 1] / (2 * np.pi))
        for rez in rez_list
    ]
    intensity_list = [
        exp_s1_q[0].params["s1_amplitude"].value / lorentz_factor_list[i] for i, exp_s1_q in enumerate(exp_s1_q_list)
    ]
    err_list = [
        exp_s1_q[0].params["s1_amplitude"].stderr / lorentz_factor_list[i] for i, exp_s1_q in enumerate(exp_s1_q_list)
    ]

    with open(file_name, "w") as f:
        f.write("Single crystal data of HoV6Sn6 (hb1a) T = 0.25K s1 scans\n")
...
if __name__ == "__main__":
    ...
    int_file = "./test_data/IPTS32912_HB1A_exp1031/FullProf/HoV6Sn6_nuc_mag.int"
