...
def analyze_scan(
    ...
    scan_data = scan.get_data(axes=("del_q(omega)", "detector"), norm_to=norm_to)
    ...
    try:  # fit to a Gaussian with a constant background
        ...
        fit = model.fit(pars, USE_ERRORBAR=False)
        fwhm = fit.params["s1_fwhm"]
    except (ValueError, KeyError) as e:
        print(f"Error fitting scan {scan_num} with hkl={hkl}: {e}")
        fit = None
    ...
    plot.add_scan(scan_data, fmt="o", color=c_data, label=f"#{scan_num} {hkl} s1 scan")
    plot.ylim = (-np.max(scan_data.y) * 0.1, np.max(scan_data.y) * 1.3)

    if fit is None:
        return fit, plot  # fitting failed, return plot with data only
...
def analyze_s1_scan_in_omega(
    ...
    try:
        ...
        print(f"Error fitting scan {scan.scan_info.scan_num} with hkl {hkl}: {e}")
        fit = None
    ...
    if plot is not None:
        ...
        plot.add_scan(scan_data, fmt="o", label=f"#{scan.scan_info.scan_num} ({h},{k},{l}) s1 scan", color=c_data)
        if fit is not None:  # fits
            fwhm = fit.params["s1_fwhm"]
            plot.add_fit(
                model,
                x=model.x_to_plot(),
...
def plot_peaks_with_resolution(
    ...
    for scan in scans_100k:
        ...
        try:
            fit, p = analyze_scan(hkl, scan, fit_range=None, plot=Plot1D(), colors=colors)
        except ValueError as e:
...
def oplot_peaks(scans_high_t: List[Scan], scans_low_t: List[Scan], pdf_path=None, colors=None):
    ...
    for scan in scans_low_t:
        ...
        p = Plot1D()
        result_s1_high_t, _, p = analyze_scan(
        ...
        result_s1_low_t, _, p = analyze_scan(hkl, scan, fit_range=None, plot=p, colors=(c_low_t_data, c_low_t_fit))
