...
def convolution(qh, en):
    ...
    while not sampled_enough:
        ...
        disp_inside = disp[np.bitwise_and(disp > min_en, disp < max_en)]
        step_en = np.max(np.mean(np.abs(np.diff(disp_inside, axis=-1)), axis=-1))

        # should be enough if step in energy is smaller than sigma/3
