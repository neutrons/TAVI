...
for i in range(len(ds)):
    ...
    newchi[i] = np.arctan2(hphi[2,i], np.sqrt(hphi[0,i]**2+hphi[1,i]**2))*180/np.pi
...
# sort = np.argsort(F2s)[::-1][:10]
# 
# hkls = hkls[sort][:10]
# ds = ds[sort][:10]
# F2s = F2s[sort][:10]
# theta2 = theta2[sort][:10]
# omega = omega[sort][:10]
# newphi = newphi[sort][:10]
# newchi = newchi[sort][:10]

sort = greedy_sort(np.column_stack([theta2, 1.5*newchi, 2*newphi]))

with open('/HFIR/HB1A/IPTS-32750/shared/matlab_scripts/hexaferrite/scanlist_Xtal_Nuc.dat', 'w') as f:
    for i in sort:
