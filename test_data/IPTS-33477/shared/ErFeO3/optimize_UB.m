...
UBmatrix=load('UBmatrix_2vec.dat','-ASCII');
InvUB=inv(UBmatrix);
lambda=2.38;
mcu=9800;
chi0=0;
...
dataList = [];
dataList2 = [];
%for scannum = [974,977,980,989,992]
for scannum = [1832:3:1880]
%for scannum = 881:1:881
    %tline = fgets(finid);
    ...
    bkgrnd,slope,area,center,width
    ...
    theta2ave = center*pi/180;
    omegaave = sum(data(:,2))/length(data(:,2))*pi/180;
    ...
    dataList = [dataList; [scannum, have, kave, lave, theta2ave*180/pi, intensity, interr, width, widtherr]]; % Append scan data to list
    dataList2 = [dataList2; [have, kave, lave, theta2ave*180/pi, omegaave*180/pi, chiave*180/pi, phiave*180/pi]];
end

%foutid = fopen('scan_optimization.dat','w');
foutid2 = fopen('fit_observe.dat','w');
%foutid2 = fopen('fit_observ.dat','w');

% Sort dataList by intensity (6th column)
%sortedData = sortrows(dataList, 6, 'descend');

%Write sorted data to file
for idx = 1:size(dataList2, 1)
    %fprintf(foutid, '%4d%4.0f%4.0f%4.0f  %8.2f %8.2f %8.2f %8.2e %8.2e\n', sortedData(idx,:));
    fprintf(foutid2, '%.0f %.0f %.0f %.2f %.2f %.2f %.2f \n', dataList2(idx,:));
end
