...
foutid=fopen('scan_magnetic_CM_300K.dat','w');

for scannum = 1212:1448
%for scannum = 1229:1229
    ...
    %[have kave lave]
    % AF4 phase
    have=round((round(H(1)*10)/10-0)*2)/2;
    kave=round((round(H(2)*10)/10-0)*2)/2;
    lave=round((round(H(3)*10)/10-1.5)*2)/2;
