...
data=load('scanlist_Xtal_mag.dat', '-ASCII');
h=data(:,1);
k=data(:,2);
l=data(:,3);
...
idx=find( (theta2>=5 & theta2<87) & ( (newchi > -70)& (newchi < 40))& (abs(theta2-61)>3 & abs(theta2-72)>3 ) );
...
length(newh)
for i=1:length(newh)
      comment=['scantitle "Magnetic_ICM_@(temp.position)K (', sprintf('%4.2f ',newh(i),newk(i),newl(i)), ')"'];
      fprintf(foutid,'%s\n',comment);
      ...
      fprintf(foutid,'%s\n',comment);
      comment = 'scan s1 @(s1)+1.35 @(s1)-1.35 0.15; drive s1 @(com)';
      fprintf(foutid,'%s\n',comment);
