...
def test_hkl_frame(component):
    sample, instrument, experiment = component
    hkl = (0, 0, 3)
    res = Resolution(ResolutionModel.CooperNathans, instrument,sample, experiment, resolution_frame = ((1, 0, 0), (0, 1, 0), (0, 0, 1), "e"))
    res_mat = res.get_resolution(hkl,4.8,4.8)
    mat = np.array(
        [
...
def test_projection_any_frame(component):
    sample, instrument, experiment = component
    hkl = (0, 0, 3)
    res = Resolution(ResolutionModel.CooperNathans, instrument, sample, experiment, resolution_frame = ((1, 1, 0), (0, 0, 1), (1, -1, 0), "en"))
    res_mat = res.get_resolution(hkl,4.8,4.8)
    mat = np.array(
        [
