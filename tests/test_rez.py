            ...
            for q3 in qe_list[2]:
                for ei0 in qe_list[3]:
                    ...
                    if rez.STATUS:
                        ...
                        if not has_axes:
                            ax = fig.add_subplot(111, axes_class=Axes, grid_helper=elps.grid_helper)
                            has_axes = True
                        elps.generate_plot(ax, c="black", linestyle="solid")
                        elps_proj = rez.generate_ellipse(axes=tuple(plot_axes), PROJECTION=True)
                        elps_proj.generate_plot(ax, c="black", linestyle="dashed")
...
def rez_plot_1D(instrument_params, xtal, projection, hkl, en, ef, R0, axis):
...
def test_1D():
    ...
    rez_plot_1D(instrument_params, test_xtal, projection, hkl, en, ef, R0, axis)
